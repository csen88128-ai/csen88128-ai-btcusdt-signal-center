...
def test_raw_store_preserves_payload_and_appends_manifest(tmp_path) -> None:
    ...
    response = RawApiResponse(
        ...
        request_url="https://fapi.binance.com/fapi/v1/markPriceKlines?interval=1m&symbol=BTCUSDT",
    ...
    assert manifest == [entry.__dict__] if hasattr(entry, "__dict__") else manifest
    assert manifest[0]["payload_sha256"] == entry.payload_sha256
